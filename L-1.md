...
L-2 spouses who do not yet have an updated I-94 evidencing employment authorization may benefit from the [180 day automatic EAD extension rule](EAD.md#what-is-the-automatic-180-day-extension-for-eads) [46], thanks to the *Shergill* settlement [48]. To be precise, if:
...
This rule is not very useful for most L-2 spouses, since most L-2 spouses have an EAD that expires on the same date as their I-94. The reason why an automatic extension cannot be granted beyond the current L-2 period of stay is that the federal regulations authorize automatic EAD extensions only when the basis for employment authorization will continue without requiring a separate adjudication [49]. If the L-2 EAD holder requires an extension of L-2 status to be granted before they can be eligible for an additional period of employment authorization, then they do not qualify for automatic extension of employment authorization. Although this rule is inconvenient, it will become obsolete soon: once L-2 spouses are able to obtain updated I-94s evidencing employment authorization, L-2 EADs will no longer be needed.
